package com.bitblocker.messenger.data;
...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!SmsHelper.isEmailAddress(number)) {
            formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164, BTBMessengerApp.getApplication().getCurrentCountryIso());
        }
    ...
    public synchronized void setNumber(String number) {
        if (!SmsHelper.isEmailAddress(number)) {
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, BTBMessengerApp.getApplication().getCurrentCountryIso());
        } else {
            mNumber = number;
        }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                String numberE164 = PhoneNumberUtils.formatNumberToE164(
                        number, BTBMessengerApp.getApplication().getCurrentCountryIso());
                String selection;
                String[] args;
